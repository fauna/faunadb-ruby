...
class ClientTest < FaunaTest # rubocop:disable Metrics/ClassLength
  ...
  def test_decode_set # rubocop:disable Metrics/AbcSize
  ...
  def test_put # rubocop:disable Metrics/AbcSize
    create_class!
  ...
  def test_logging # rubocop:disable Metrics/MethodLength
    _, err = capture_io do
      test_client = stub_client(
        :get,
        'ping',
  ...
  Response JSON: {
    ...
    assert(/^  Response \(200\): API processing 123ms, network latency \dms$/ =~ lines[-1])
  end
